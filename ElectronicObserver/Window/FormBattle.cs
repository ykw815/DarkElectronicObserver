	...
	public partial class FormBattle : DockContent
	{
		...
		private void SetFormation(BattleManager bm)
		{
			...
			if (bm.IsEnemyCombined && bm.StartsFromDayBattle)
			{
				...
				FleetEnemy.BackColor = willMain ? ColorTranslator.FromHtml("#2A2A4D") : ColorTranslator.FromHtml("#2A2A2D");
				FleetEnemyEscort.BackColor = willMain ? ColorTranslator.FromHtml("#2A2A2D") : ColorTranslator.FromHtml("#2A2A4D");
			}
			else
			{
				FleetEnemy.BackColor =
				FleetEnemyEscort.BackColor = ColorTranslator.FromHtml("#2A2A2D");
			}
        }

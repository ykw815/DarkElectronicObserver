...
namespace ElectronicObserver.Window
{
    ...
    public partial class FormCompass : DockContent
    {
        ...
        private class TableEnemyMemberControl : IDisposable
        {
            ...
            public void Update(int shipID, int[] slot)
            {
                ...
                if (shipID == -1)
                {
                    //なし
                    ShipName.Text = "-";
                    ShipName.ForeColor = ColorTranslator.FromHtml("#D0D0D0");ShipName.ForeColor = ColorTranslator.FromHtml("#D0D0D0");
            ...
            public void Update(EnemyFleetRecord.EnemyFleetElement fleet)
            {
                ...
                for (int i = 0; i < 6; i++)
                {
                    ...
                    if (ship == null)
                    {
                        // nothing
                        ShipNames[i].Text = "-";
                        ShipNames[i].ForeColor = ColorTranslator.FromHtml("#D0D0D0");ShipNames[i].ForeColor = ColorTranslator.FromHtml("#D0D0D0");
                        ShipNames[i].Tag = -1;
        ...
        public FormCompass(FormMain parent)
        {
            ...
            MainFontColor = ColorTranslator.FromHtml("#D0D0D0");MainFontColor = ColorTranslator.FromHtml("#D0D0D0");
